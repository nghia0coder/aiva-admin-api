...
public class StreamDataChatHandler(
...
{
  public async ValueTask<Result<StreamDataChatResponse>> Handle(StreamDataChatCommand request, CancellationToken cancellationToken)
  {
    try
    {
      ...
      var response = new StreamDataChatResponse();
    ...
    {
      logger.LogError(ex, "Error processing data chat stream for conversation {ConversationId}", request.ConversationId);
